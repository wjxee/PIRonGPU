...
//     }
    ...
    void publickey_gen_cpu(
        Data64* public_key, Data64* secret_key, Data64* error_poly, Data64* a_poly,
        Modulus64* modulus, int n_power, int rns_mod_count)
    {
        int n = 1 << n_power; // 计算环的大小
        
        // 遍历所有模数和环元素
        for (int block_y = 0; block_y < rns_mod_count; block_y++) {
            for (int idx = 0; idx < n; idx++) {
                int location = idx + (block_y << n_power);
                ...
                Data64 temp = OPERATOR64::mult(sk, a, modulus[block_y]);
                temp = OPERATOR64::add(temp, e, modulus[block_y]);
                
                Data64 zero = 0ULL;
                temp = OPERATOR64::sub(zero, temp, modulus[block_y]);
