...
namespace heoncpu
{
    HEKeyGenerator::HEKeyGenerator(Parameters& context)
    {
    ...
    void HEKeyGenerator::generate_secret_key(Secretkey& sk
                                                      )
    {
        if (sk.secretkey_.size() < n)
        {
            sk.secretkey_ = std::vector<int>(n);
        }
        ...
        {
            sk.location_ = std::vector<Data64>(Q_prime_size_ * n);
        }
        ...
        GPU_NTT_Inplace<Data64>(sk.data(), ntt_table_->data(), modulus_->data(),
    ...
    void HEKeyGenerator::generate_public_key(Publickey& pk, Secretkey& sk)
    {
        if (sk.location_.size() < (Q_prime_size_ * n))
        {
        ...
        {
            pk.locations_ = std::vector<Data64>(2 * Q_prime_size_ * n);
        }
        ...
        GPU_NTT_Inplace<Data64>(errors_a.data(), ntt_table_->data(), modulus_->data(),
                        cfg_ntt, Q_prime_size_, Q_prime_size_);
        // HEONGPU_CUDA_CHECK(cudaGetLastError());
        ...
        // HEONGPU_CUDA_CHECK(cudaGetLastError());
        publickey_gen_cpu(
            pk.data(), sk.data(), error_poly, a_poly, modulus_->data(), n_power, Q_prime_size_);    
        pk.in_ntt_domain_ = true;
    }

    // void HEKeyGenerator::generate_multi_party_public_key_piece(
    ...
    // void HEKeyGenerator::generate_multi_party_public_key(
    ...
    //     MultipartyRelinkey& rk, Secretkey& sk, const )
    // {
    //     if (sk.location_.size() < (Q_prime_size_ * n))
    //     {
    //         throw std::invalid_argument(
    ...
    //     std::vector<Data64> random_values(
    ...
    //     rk.device_location_ = std::vector<Data64>(rk.relinkey_size_);
    ...
    //         std::vector<Data64>(rk_stage_2.relinkey_size_);
    ...
    //     else
    //     {
    //         std::vector<Data64> temp_location(rk_stage_1.device_location_,
    //                                            stream);
    //         multi_party_relinkey_piece_method_I_II_stage_II_kernel<<<
    //             dim3((n >> 8), Q_prime_size_, 1), 256, 0>>>(
    ...
    //     std::vector<Data64> random_values(Q_prime_size_ * ((3 * d_) + 1) * n,
    //                                        stream);
    //     Data64* e0 = random_values.data();
    ...
    //     std::vector<Data64> random_values(Q_prime_size_ * ((2 * d_) + 1) * n,
    //                                        stream);
    //     Data64* e0 = random_values.data();
    //     Data64* e1 = e0 + (Q_prime_size_ * d_ * n);
    //     Data64* u = e1 + (Q_prime_size_ * d_ * n);
    ...
    //     modular_gaussian_random_number_generation_kernel<<<
    //         dim3((n >> 8), 2 * d_leveled_->operator[](0), 1), 256, 0>>>(
    //         e0, modulus_->data(), n_power, Q_prime_size_, seed_, offset_);
    //     HEONGPU_CUDA_CHECK(cudaGetLastError());
    //     offset_++;
    ...
    // void HEKeyGenerator::generate_multi_party_relin_key(
    ...
    // void HEKeyGenerator::generate_bfv_relin_key_method_II(
    //     Relinkey& rk, Secretkey& sk, const )
    // {
    ...
    // void HEKeyGenerator::generate_bfv_relin_key_method_III(
    //     Relinkey& rk, Secretkey& sk, const )
    // {
    //     std::vector<Data64> errors_a(2 * Q_prime_size_ * d_ * n);
    //     Data64* error_poly = errors_a.data();
    ...
    //     std::vector<Data64> temp_calculation(2 * Q_prime_size_ * d_ * n,
    //                                           stream);
    ...
    // void HEKeyGenerator::generate_ckks_relin_key_method_II(
    //     Relinkey& rk, Secretkey& sk, const )
    // {
    //     std::vector<Data64> errors_a(
    ...
    // void HEKeyGenerator::generate_ckks_relin_key_method_III(
    //     Relinkey& rk, Secretkey& sk, const )
    // {
    //     int max_depth = Q_size_ - 1;
    //     std::vector<Data64> temp_calculation(
    //         2 * Q_prime_size_ * d_leveled_->operator[](0) * n);
    //     std::vector<Data64> errors_a(
    //         2 * Q_prime_size_ * d_leveled_->operator[](0) * n);
    //     Data64* error_poly = errors_a.data();
    ...
    //             std::vector<Data64>(rk.relinkey_size_leveled_[i]));
    ...
    HEKeyGenerator::generate_galois_key_method_I(Galoiskey& gk, Secretkey& sk )
    {
        ...
        if (!gk.customized)
        {
            ...
            for (auto& galois : gk.galois_elt)
            {
                ...
                gk.host_location_[galois.second] =
                    std::vector<Data64>(gk.galoiskey_size_);
                // galoiskey_gen_kernel<<<dim3((n >> 8), Q_prime_size_, 1), 256, 0,
                //                        stream>>>(
                //     gk.host_location_[galois.second].data(), sk.data(),
                //     error_poly, a_poly, modulus_->data(), factor_->data(),
                //     inv_galois, n_power, Q_prime_size_);
                // HEONGPU_CUDA_CHECK(cudaGetLastError());
                galoiskey_gen_cpu(
                    gk.host_location_[galois.second].data(), sk.data(), error_poly, a_poly,
                    modulus_->data(), factor_->data(), inv_galois, n_power, Q_prime_size_,
                    n >> 8, Q_prime_size_, 1, 256 // grid_x, grid_y, grid_z, block_size
                );
            }
            ...
            // galoiskey_gen_kernel<<<dim3((n >> 8), Q_prime_size_, 1), 256, 0,
            //                        stream>>>(
            //     gk.zero_host_location_.data(), sk.data(), error_poly, a_poly,
            //     modulus_->data(), factor_->data(), gk.galois_elt_zero, n_power,
            //     Q_prime_size_);
            // HEONGPU_CUDA_CHECK(cudaGetLastError());
            galoiskey_gen_cpu(
                gk.zero_host_location_.data(), sk.data(), error_poly, a_poly,
                modulus_->data(), factor_->data(), gk.galois_elt_zero, n_power, Q_prime_size_,
                n >> 8, Q_prime_size_, 1, 256 // grid_x, grid_y, grid_z, block_size
            );
        ...
        else //here
        {
            for (auto& galois_ : gk.custom_galois_elt)
            {
                ...
                // HEONGPU_CUDA_CHECK(cudaGetLastError());
                galoiskey_gen_cpu(
                    gk.host_location_[galois_].data(), sk.data(), error_poly, a_poly,
                    modulus_->data(), factor_->data(), inv_galois, n_power, Q_prime_size_,
                    n >> 8, Q_prime_size_, 1, 256 // grid_x, grid_y, grid_z, block_size
                );
    ...
    void HEKeyGenerator::generate_bfv_galois_key_method_II(
        Galoiskey& gk, Secretkey& sk)
    {
        ...
        if (!gk.customized)
        {
            ...
            for (auto& galois : gk.galois_elt)
            {
                ...
                //     Q_size_, P_size_);
                // HEONGPU_CUDA_CHECK(cudaGetLastError());
                galoiskey_gen_II_cpu(
                    gk.host_location_[galois.second].data(), sk.data(), error_poly, a_poly,
                    modulus_->data(), factor_->data(), inv_galois, Sk_pair_->data(), n_power,
                    Q_prime_size_, d_, Q_size_, P_size_,
                    n >> 8, Q_prime_size_, 1, 256 // grid_x, grid_y, grid_z, block_size
                );
            }
            ...
            galoiskey_gen_II_cpu(
                gk.zero_host_location_.data(), sk.data(), error_poly, a_poly,
                modulus_->data(), factor_->data(), gk.galois_elt_zero, Sk_pair_->data(), n_power,
                Q_prime_size_, d_, Q_size_, P_size_,
                n >> 8, Q_prime_size_, 1, 256 // grid_x, grid_y, grid_z, block_size
            );                            
            // if (gk.store_in_gpu_)
    ...
    // void HEKeyGenerator::generate_ckks_galois_key_method_II(
    //     Galoiskey& gk, Secretkey& sk, const )
    // {
    //     std::vector<Data64> errors_a(
    //         2 * Q_prime_size_ * d_leveled_->operator[](0) * n);
    //     Data64* error_poly = errors_a.data();
    ...
    //     MultipartyGaloiskey& gk, Secretkey& sk, const )
    // {
    //     if (sk.location_.size() < (Q_prime_size_ * n))
    //     {
    //         throw std::invalid_argument(
    ...
    // void HEKeyGenerator::generate_multi_party_galois_key(
    ...
    // void HEKeyGenerator::generate_switch_key_method_I(
    ...
    //         std::vector<Data64>(swk.switchkey_size_);
    ...
    // void HEKeyGenerator::generate_bfv_switch_key_method_II(
    //     Switchkey& swk, Secretkey& new_sk, Secretkey& old_sk,
    //     const )
    // {
    //     std::vector<Data64> errors_a(2 * Q_prime_size_ * d_ * n);
    ...
    // void HEKeyGenerator::generate_ckks_switch_key_method_II(
    //     Switchkey& swk, Secretkey& new_sk, Secretkey& old_sk,
    //     const )
    // {
    //     std::vector<Data64> errors_a(
