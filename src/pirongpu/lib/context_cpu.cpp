...
namespace heoncpu
{
    ...
    void Parameters::print_parameters()
    {
        ...
        switch (scheme_)
        {
            case heoncpu::scheme_type::bfv:
                scheme_string = "BFV";
                break;
            case heoncpu::scheme_type::ckks:
                scheme_string = "CKKS";
                break;
            default:
        ...
        if (scheme_ == heoncpu::scheme_type::bfv)
        {
    ...
    void Parameters::generate()
    {
        ...
        modulus_ = std::make_shared<std::vector<Modulus64>>(prime_vector);
        ...
        ntt_table_ = std::make_shared<std::vector<Root64>>(Qprime_ntt_table);

        intt_table_ = std::make_shared<std::vector<Root64>>(Qprime_intt_table);

        n_inverse_ =
            std::make_shared<std::vector<Ninverse64>>(Qprime_n_inverse);
        ...
        last_q_modinv_ = std::make_shared<std::vector<Data64>>(last_q_modinv);

        half_p_ = std::make_shared<std::vector<Data64>>(half);

        half_mod_ = std::make_shared<std::vector<Data64>>(half_mod);

        factor_ = std::make_shared<std::vector<Data64>>(factor);
        ...
        if (scheme_ == scheme_type::bfv)
        {
            ...
            Mi_ = std::make_shared<std::vector<Data64>>(Mi);

            Mi_inv_ = std::make_shared<std::vector<Data64>>(Mi_inv);

            upper_half_threshold_ =
                std::make_shared<std::vector<Data64>>(upper_half_threshold);

            decryption_modulus_ =
                std::make_shared<std::vector<Data64>>(decryption_modulus);
            ...
            base_Bsk_ = std::make_shared<std::vector<Modulus64>>(base_Bsk_mod);

            bsk_ntt_tables_ =
                std::make_shared<std::vector<Root64>>(Bsk_ntt_table);

            bsk_intt_tables_ =
                std::make_shared<std::vector<Root64>>(Bsk_intt_table);

            bsk_n_inverse_ =
                std::make_shared<std::vector<Ninverse64>>(Bsk_n_inverse);
            ...
            base_change_matrix_Bsk_ =
                std::make_shared<std::vector<Data64>>(base_matrix_q_Bsk);

            inv_punctured_prod_mod_base_array_ =
                std::make_shared<std::vector<Data64>>(
            ...
            inv_m_tilde_mod_Bsk_ =
                std::make_shared<std::vector<Data64>>(inv_m_tilde_mod_Bsk);

            prod_q_mod_Bsk_ =
                std::make_shared<std::vector<Data64>>(prod_q_mod_Bsk);

            inv_prod_q_mod_Bsk_ =
                std::make_shared<std::vector<Data64>>(inv_prod_q_mod_Bsk);

            base_change_matrix_q_ =
                std::make_shared<std::vector<Data64>>(base_matrix_Bsk_q);

            base_change_matrix_msk_ =
                std::make_shared<std::vector<Data64>>(base_change_matrix_msk);
            ...
            prod_B_mod_q_ =
                std::make_shared<std::vector<Data64>>(prod_B_mod_q);

            q_Bsk_merge_modulus_ =
                std::make_shared<std::vector<Modulus64>>(q_Bsk_merge_modulus);

            q_Bsk_merge_ntt_tables_ =
                std::make_shared<std::vector<Root64>>(q_Bsk_forward_tables);

            q_Bsk_merge_intt_tables_ =
                std::make_shared<std::vector<Root64>>(q_Bsk_inverse_tables);

            q_Bsk_n_inverse_ =
                std::make_shared<std::vector<Ninverse64>>(q_Bsk_n_inverse);

            plain_modulus2_ =
                std::make_shared<std::vector<Modulus64>>(plain_mod2);

            n_plain_inverse_ =
                std::make_shared<std::vector<Ninverse64>>(plain_n_inverse);

            plain_ntt_tables_ =
                std::make_shared<std::vector<Root64>>(plain_forward_table);

            plain_intt_tables_ =
                std::make_shared<std::vector<Root64>>(plain_inverse_table);

            gamma_ = gamma_mod;

            coeeff_div_plainmod_ =
                std::make_shared<std::vector<Data64>>(coeff_div_plain_modulus);

            Q_mod_t_ = Q_mod_t;

            upper_threshold_ = plain_upper_half_threshold;

            upper_halfincrement_ = std::make_shared<std::vector<Data64>>(
                plain_upper_half_increment);

            Qi_t_ = std::make_shared<std::vector<Data64>>(Qi_t);

            Qi_gamma_ = std::make_shared<std::vector<Data64>>(Qi_gamma);

            Qi_inverse_ = std::make_shared<std::vector<Data64>>(Qi_inverse);
        ...
        else if (scheme_ == scheme_type::ckks)
        {
            ...
            rescaled_last_q_modinv_ =
                std::make_shared<std::vector<Data64>>(rescale_last_q_modinv);

            rescaled_half_mod_ =
                std::make_shared<std::vector<Data64>>(rescaled_half_mod);

            rescaled_half_ =
                std::make_shared<std::vector<Data64>>(rescaled_half);
            ...
            prime_location_leveled =
                std::make_shared<std::vector<int>>(prime_loc);
        }
        ...
        switch (static_cast<int>(keyswitching_type_))
        {
            ...
            case 2: // KEYSWITCHING_METHOD_II
                ...
                if (scheme_ == scheme_type::bfv)
                {
                    ...
                    prod_D_to_Q_tilda_ = std::make_shared<std::vector<Data64>>(
                        prod_D_to_Q_tilda_inner);

                    std::vector<int> I_j_inner = pool.I_j();
                    I_j_ = std::make_shared<std::vector<int>>(I_j_inner);

                    std::vector<int> I_location_inner = pool.I_location();
                    I_location_ =
                        std::make_shared<std::vector<int>>(I_location_inner);

                    std::vector<int> Sk_pair_inner = pool.sk_pair();
                    Sk_pair_ =
                        std::make_shared<std::vector<int>>(Sk_pair_inner);
                }
                else if (scheme_ == scheme_type::ckks)
                {
                    ...
                    base_change_matrix_D_to_Qtilda_leveled =
                        std::make_shared<std::vector<std::vector<Data64>>>();
                         ...
                         pool_lp++)
                    {
                        ...
                        std::vector<Data64> Mi_inv_D_to_Qtilda_leveled_inner(
                            Mi_inv_D_to_Qtilda_vec[pool_lp]);
                        std::vector<Data64> prod_D_to_Qtilda_leveled_inner(
                        ...
                        std::vector<int> I_j_vec_inner(I_j_vec[pool_lp]);
                        std::vector<int> I_location_vec_inner(
                            I_location_vec[pool_lp]);
                        std::vector<int> Sk_pair_new_vec_inner(
            ...
            case 3: // KEYSWITCHING_METHOD_III
                ...
                if (scheme_ == scheme_type::bfv)
                {
                    ...
                    B_prime_ = std::make_shared<std::vector<Modulus64>>(
                        B_prime_inner);

                    std::vector<Root64> B_prime_ntt_tables_inner =
                        pool.B_prime_ntt_tables();
                    B_prime_ntt_tables_ =
                        std::make_shared<std::vector<Root64>>(
                    ...
                    B_prime_n_inverse_ =
                        std::make_shared<std::vector<Ninverse64>>(
                    ...
                    std::vector<Data64> Mi_inv_D_to_B_inner =
                        pool.Mi_inv_D_to_B();
                    Mi_inv_D_to_B_ = std::make_shared<std::vector<Data64>>(
                        Mi_inv_D_to_B_inner);

                    std::vector<Data64> Mi_inv_B_to_D_inner =
                        pool.Mi_inv_B_to_D();
                    Mi_inv_B_to_D_ = std::make_shared<std::vector<Data64>>(
                        Mi_inv_B_to_D_inner);

                    std::vector<Data64> prod_D_to_B_inner = pool.prod_D_to_B();
                    prod_D_to_B_ = std::make_shared<std::vector<Data64>>(
                        prod_D_to_B_inner);

                    std::vector<Data64> prod_B_to_D_inner = pool.prod_B_to_D();
                    prod_B_to_D_ = std::make_shared<std::vector<Data64>>(
                        prod_B_to_D_inner);
                ...
                else if (scheme_ == scheme_type::ckks)
                {
                    ...
                    B_prime_leveled = std::make_shared<std::vector<Modulus64>>(
                        B_prime_inner);
                         ...
                         pool_lp++)
                    {
                        ...
                        std::vector<Data64> Mi_inv_D_to_B_leveled_inner(
                            Mi_inv_D_to_B_vec[pool_lp]);
                        std::vector<Data64> prod_D_to_B_leveled_inner(
                            prod_D_to_B_vec[pool_lp]);
                        std::vector<Data64> prod_B_to_D_leveled_inner(
